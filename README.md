...
## Struktur proyek
- [TrainLSTM.ipynb](TrainLSTM.ipynb) — Notebook eksperimental.
...
## Penyiapan data
  ...
  - Pastikan file .mat peserta berada di folder yang sesuai (default masing‑masing skrip menunjuk ke `CS_Train` via [`dataset_dir`](https://www.google.com/search?q=TrainLSTM1.py)).
  - Jika ingin mengabaikan file .mat di repo, periksa [`.gitignore`](https://www.google.com/search?q=.gitignore) dan jika file .mat sudah ter-track: gunakan `git rm --cached <path>` lalu commit. Pertimbangkan Git LFS untuk file besar.
...
```
  ...
  - Periksa parameter seperti batch\_size, epochs, dan early stopping di tiap skrip.

## Penjelasan singkat skrip

  - Semua skrip:
      - Membaca .mat dengan key `data`, `valence_labels`, `arousal_labels`, `dominance_labels`.
      - Mengubah bentuk X ke (12, 5, 64) dan menggabungkan label 3-bit menjadi kelas aksara via [`enc_lbl`](https://www.google.com/search?q=TrainLSTM1.py).
      - Melakukan 10‑fold KFold CV dan mencetak metrik: accuracy, precision, recall, F1, epoch stats.
  - Perbedaan skenario:
      - [TrainLSTM1.py](https://www.google.com/search?q=TrainLSTM1.py): single LSTM layer.
      - [TrainLSTM2.py](https://www.google.com/search?q=TrainLSTM2.py): stacked LSTM 64 → 32.
      - [TrainLSTM3.py](https://www.google.com/search?q=TrainLSTM3.py): stacked LSTM 32 → 32 + dropout.

---

Author: I Dewa Gede Mahesta Parawangsa  
Contact: dewamahesta2711@gmail.com
